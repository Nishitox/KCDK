function kcdk:score/initialize
function kcdk:score/detect_version

function kcdk:common/default_settings/difficulty
function kcdk:common/default_settings/gamerules
function kcdk:common/default_settings/time
function kcdk:common/default_settings/weather
function kcdk:common/default_settings/worldborder
function kcdk:common/default_settings/worldspawn

function kcdk:common/initial_settings/gamerules
function kcdk:bossbar/add/general
function kcdk:team/initialize
function kcdk:message/lang
function kcdk:ptp/setup

function kcdk:message/tellraw/announce/setup_with_formatting_executed